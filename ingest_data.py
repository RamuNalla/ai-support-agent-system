...
from agent_service.app.rag.embeddings import EmbeddingsGenerator                # Your custom module for embeddings
from agent_service.app.rag.vector_db import VectorDBManager                     # Your custom module for FAISS
from uuid import uuid4                                                          # To generate unique IDs for document chunks

logging.basicConfig(level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s')
logger = logging.getLogger(__name__)

DOCS_PATH = "agent_service/data/knowledge_base/docs"            # Path to your knowledge base documents 
FAISS_INDEX_FILE = "agent_service/faiss_index.bin"              # FAISS settings: This is where the generated FAISS index will be saved.
...
def ingest_documents():             # Loads documents from the specified path, splits them into chunks, generates embeddings for each chunk, and upserts them into the FAISS index, then saves the index to disk for persistence.
    ...
    try:
        loader = DirectoryLoader(DOCS_PATH, glob="**/*.md", loader_cls=TextLoader)      # DirectoryLoader can load multiple markdown (.md) files from a directory.
